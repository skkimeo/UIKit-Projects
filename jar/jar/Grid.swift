...
struct Grid {
    ...
    /// 그리드를 만들 영역의 프레임
    /// 변경 시 셀 크기가 자동으로 조정됨
    var frame: CGRect { didSet { self.calculate() }}
    ...
    /// 셀 크기
    var cellSize: CGSize {
        self.cellFrames.first?.size ?? .zero
    }
    ...
    init(frame: CGRect = .zero, cellCount: Int = 0, aspectRatio: CGFloat = 1) {
        self.frame = frame
...
//                + offset.dx
    ...
    private func largestCellSizeThatFits() -> CGSize {
        guard self.cellCount > 0 && self.aspectRatio > 0
        else { return .zero }
